    ...
    */
    private function generateCensorChecks($fullWords = false) {
        ...
        for ($x = 0, $xMax = count($badwords); $x < $xMax; $x++) {
            ...
            ? '/\b' . str_ireplace(array_keys($leet_replace), array_values($leet_replace), $badwords[$x]) . '\b/ui'
            : '/'   . str_ireplace(array_keys($leet_replace), array_values($leet_replace), $badwords[$x]) . '/ui';
        }
