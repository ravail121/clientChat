    ...
    if (!empty($load['group_id']) && !empty($load['user_id'])) {
        ...
        if (isset($group_info[0])) {
            ...
            if ($super_privileges || isset($group_info['current_user_group_role_id']) && !empty($group_info['current_user_group_role_id'])) {
                ...
                if (!$super_privileges) {
                    $columns = $where = $join = null;
                    $where["group_roles.group_role_id"] = $group_info['current_user_group_role_id'];
                    $current_user = DB::connect()->select('group_roles', ['group_role_attribute', 'role_hierarchy'], $where);
                ...
                if (isset($current_user['group_role_attribute']) && $current_user['group_role_attribute'] !== 'administrators') {
                    if (isset($current_user['role_hierarchy'])) {
                        $where["group_roles.role_hierarchy[<]"] = $current_user['role_hierarchy'];
                    }
                }
