...
if (role(['permissions' => ['super_privileges' => 'view_statistics']])) {
    ...
    if (role(['permissions' => ['memberships' => 'view_site_transactions']]) && $load === 'memberships') {
        ...
        if (Registry::load('settings')->cloud_storage !== 'disable') {
            $total_size = cloud_storage_module(['get_size' => 'assets/files/storage/', 'load_from_cache' => true]);
            $items[$child_index]['result'] = files('getsize', ['convert_size' => $total_size]);
        } else {
