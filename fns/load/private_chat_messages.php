        ...
        if (!empty($data["user_id"])) {
            ...
            if ($data["user_id"] === 'all' || isset($private_conversation[0]['private_conversation_id'])) {
                ...
                $columns = [
                    ...
                    'message_author_site_role.site_role_id', 'message_author.profile_picture',
                ];
                ...
                foreach ($private_chat_messages as $message) {
                    ...
                    if (isset($message["system_message"]) && !empty($message["system_message"])) {
                        ...
                        if ($data["user_id"] !== 'all') {
                            ...
                            $output['messages'][$i]->options[1] = new stdClass();
                            $output['messages'][$i]->options[1]->option = Registry::load('strings')->sender;
                            $output['messages'][$i]->options[1]->class = 'get_info';
                            $output['messages'][$i]->options[1]->attributes['user_id'] = $message['user_id'];

                            $output['messages'][$i]->options[3] = new stdClass();
                            $output['messages'][$i]->options[3]->option = Registry::load('strings')->recipient;
                            $output['messages'][$i]->options[3]->class = 'get_info';
                            $output['messages'][$i]->options[3]->attributes['user_id'] = $recipient_user_id;
                        }
                        ...
                        if ($allow_edit_message) {
                            ...
                            $output['messages'][$i]->options[2]->option = Registry::load('strings')->edit;
                            $output['messages'][$i]->options[2]->class = 'load_form';
                            $output['messages'][$i]->options[2]->attributes['enlarge'] = true;
                            $output['messages'][$i]->options[2]->attributes['data-message_id'] = $message['private_chat_message_id'];
                            $output['messages'][$i]->options[2]->attributes['form'] = 'private_chat_messages';

                            if ($data["user_id"] === 'all') {
                                $output['messages'][$i]->options[2]->attributes['data-monitoring_chat'] = true;
                            }
                        }

                        if ($allow_delete_message) {
                            $output['messages'][$i]->options[5] = new stdClass();
                            $output['messages'][$i]->options[5]->option = Registry::load('strings')->delete;
                            $output['messages'][$i]->options[5]->class = 'ask_confirmation';
                            $output['messages'][$i]->options[5]->attributes['data-remove'] = 'private_chat_messages';
                            $output['messages'][$i]->options[5]->attributes['data-message_id'] = $message['private_chat_message_id'];
                            $output['messages'][$i]->options[5]->attributes['confirmation'] = Registry::load('strings')->confirm_delete;
                            $output['messages'][$i]->options[5]->attributes['submit_button'] = Registry::load('strings')->yes;
                            $output['messages'][$i]->options[5]->attributes['cancel_button'] = Registry::load('strings')->no;
                            $output['messages'][$i]->options[5]->attributes['column'] = 'second';
                        }
                    }
