    ...
    if (!empty($badge_id)) {
        ...
        if (isset($badge['badge_category']) && $badge['badge_category'] === 'profile') {
            $columns = [
                ...
                'badges_assigned.badge_assigned_id', 'site_users.profile_picture(image)'
            ];
            ...
            $columns = [
                ...
                'badges_assigned.badge_assigned_id', 'groups.group_picture(image)'
            ];
        ...
        foreach ($assigned_badges as $assigned_badge) {
            ...
            if (isset($assigned_badge['badge_assigned_id']) && !empty($assigned_badge['badge_assigned_id'])) {
                ...
                if (isset($badge['badge_category']) && $badge['badge_category'] === 'profile') {
                    $output['content'][$i]->image = get_img_url(['from' => 'site_users/profile_pics', 'image' => $assigned_badge['image']]);

                } else {
                    $output['content'][$i]->image = get_img_url(['from' => 'groups/icons', 'image' => $assigned_badge['image']]);
                }
