...
if (role(['permissions' => ['site_users' => 'block_users']])) {
    ...
    $columns = [
        ...
        'site_users.username', 'site_users_blacklist.user_blacklist_id', 'site_users.profile_picture'
    ];

    $join["[>]site_users"] = ["site_users_blacklist.blacklisted_user_id" => "user_id"];
    $where["site_users_blacklist.user_id"] = $user_id;
    $where["site_users_blacklist.block"] = 1;
    ...
    $output['loaded']->title = Registry::load('strings')->blocked;
    ...
    $output['sortby'][3]->attributes['load'] = 'blocked';
    ...
    foreach ($site_users as $user) {
        ...
        $output['options'][$i][1]->option = Registry::load('strings')->unblock;
        $output['options'][$i][1]->class = 'ask_confirmation';
        $output['options'][$i][1]->attributes['data-update'] = 'site_user_blacklist';
        $output['options'][$i][1]->attributes['data-unblock_user_id'] = $user['user_id'];
        $output['options'][$i][1]->attributes['confirmation'] = Registry::load('strings')->unblock_user_confirmation;
