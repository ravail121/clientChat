...
if (Registry::load('settings')->friend_system === 'enable') {
    if (role(['permissions' => ['friend_system' => 'view_friends']])) {
        ...
        $columns = [
            'from_user.display_name(from_fullname)', 'from_user.email_address(from_email)', 'from_user.site_role_id(from_site_role)',
            'from_user.username(from_username)', 'from_user.online_status(from_online)', 'friends.friendship_id',
            'friends.from_user_id', 'friends.to_user_id', 'friends.relation_status', 'from_settings.offline_mode(from_offline_mode)',
            ...
            'to_settings.offline_mode(to_offline_mode)', 'from_user.profile_picture(from_picture)',
            'to_user.profile_picture(to_picture)', 'to_user.site_role_id(to_site_role)',
        ];

        $join["[>]site_users(from_user)"] = ["friends.from_user_id" => "user_id"];
        $join["[>]site_users(to_user)"] = ["friends.to_user_id" => "user_id"];

        $join["[>]site_users_settings(from_settings)"] = ["friends.from_user_id" => "user_id"];
        $join["[>]site_users_settings(to_settings)"] = ["friends.to_user_id" => "user_id"];
        ...
        foreach ($site_users as $user) {
            ...
            if ((int)$user['from_user_id'] === (int)$current_user_id) {
                ...
                $user['profile_picture'] = $user['to_picture'];
                $user['site_role'] = $user['to_site_role'];
            } else {
                ...
                $user['profile_picture'] = $user['from_picture'];
                $user['site_role'] = $user['from_site_role'];
            }
            ...
            if ($pm_only_specific_roles === 'yes') {
                ...
                $user_site_role = (int)$user['site_role'];
